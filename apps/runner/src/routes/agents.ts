...
export function registerAgentRoutes(app: FastifyInstance, deps: AgentRoutesDeps): void {
    ...
    registerAsksStreamRoute(app, deps);
}
...
interface AsksStreamParams {
    id: string;
}

interface StartedAskInfo {
    askCallId: string;
    callerNodeId: string;
    calleeNodeId: string;
    startedAt: number;
...
interface AsksCompletedPayload {
    eventId: string;
    askCallId: string;
    status: 'ok' | 'failed';
...
function seedStartedFromBus(
    bus: EventBus,
    ownNodeIds: ReadonlySet<string>,
): Map<string, StartedAskInfo> {
    const seeded = new Map<string, StartedAskInfo>();
    const rootIds = bus.rootEventIdsBySourcePrefix('ask:');
    for (const rootId of rootIds) {
        const events = bus.eventsByDispatch(rootId);
        let root: DispatchEvent | undefined;
        for (const ev of events) {
            if (!('type' in ev) && ev.eventId === rootId && !ev.parentId) {
                root = ev;
                break;
            }
        }
        ...
        const parts = readAskParts(root.meta);
        if (!parts) continue;
        if (!ownNodeIds.has(parts.callerNodeId)) continue;
        const askCallId =
            root.meta && typeof root.meta.ask_call_id === 'string'
                ? (root.meta.ask_call_id as string)
                : '';
        if (!askCallId) continue;
        seeded.set(rootId, {
            askCallId,
            callerNodeId: parts.callerNodeId,
            calleeNodeId: parts.calleeNodeId,
...
function registerAsksStreamRoute(app: FastifyInstance, deps: AgentRoutesDeps): void {
    app.get<{ Params: AsksStreamParams }>(
        '/graphs/:id/asks/stream',
        (req: FastifyRequest<{ Params: AsksStreamParams }>, reply: FastifyReply) => {
            const { id } = req.params;
            ...
            const ownNodeIds = new Set(loaded.graph.nodes.map((n) => n.id));
            ...
            const started = seedStartedFromBus(deps.bus, ownNodeIds);
            ...
            const offDispatch = deps.bus.subscribeDispatch((ev) => {
                ...
                started.set(ev.eventId, {
                    askCallId,
                    callerNodeId,
                    calleeNodeId,
                    startedAt,
                });
                const brief = snippet(firstUserContent(ev.messages));
                const payload: AsksStartedPayload = {
                    eventId: ev.eventId,
                    askCallId,
                    callerNodeId,
                    calleeNodeId,
