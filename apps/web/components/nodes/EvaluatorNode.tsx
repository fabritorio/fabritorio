...
import type { EvaluatorNode as EvaluatorNodeData } from '@fabritorio/types';
import type { NodeExecState } from '@/lib/node-state';
import { nodeStateClassName } from '@/lib/node-style';

type Props = NodeProps & {
    data: Partial<EvaluatorNodeData> & { __state?: NodeExecState };
};

export function EvaluatorNode({ data }: Props) {
    const state = data.__state ?? 'idle';
    return (
        <div className={`min-w-[200px] ${nodeStateClassName('evaluator', state)}`}>
            ...
            <div className="fab-node-footer text-xs text-amber-700 dark:text-amber-200">
                branch · tools? loop : done
            </div>
